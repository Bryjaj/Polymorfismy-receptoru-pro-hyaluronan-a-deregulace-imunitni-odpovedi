...
ConSurf <- read.xlsx("ConSurf_LYVE1.xlsx", sheetName = "List1")
ConSurf <- ConSurf %>% 
...
GnomAD <- read.csv("GnomAD_LYVE1.csv")
```
...
Po manuální prohlídce této proměnné je třeba provést manuální úpravy, protože HGVS kód může obsahovat další přídatná čísla nesouvisející s pozicí mutace. Příkaz `head()` ukazuje pouze prvních 6 řádků kontrolního data framu. Při kontrole je však třeba prohlédnout celý data frame.
```{r genposCheck}
head(data.frame(Genpos = GnomAD$genepos, 
                HGVS = GnomAD$HGVS.Consequence))
```

V první mutaci se nachází kromě pozice ještě číslo 5. Tuto pozici je tedy třeba upravit.
```{r upravyGenpos}
GnomAD$genepos[1] <- 323
```
